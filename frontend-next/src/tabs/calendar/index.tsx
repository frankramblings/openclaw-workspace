...
import { addDays, eventDays, isoDay, localDateTime, monthWindow, withLocalOffset } from './logic'
...
export function CalendarTab() {
  ...
  return <main className="next-tab next-calendar-tab"><SectionHeader title="Calendar" actions={<><Button variant="ghost" onClick={() => void store.shift(-1)}>Previous</Button><Button variant="ghost" onClick={() => void store.load(0)}>Today</Button><Button variant="ghost" onClick={() => void store.shift(1)}>Next</Button><Button onClick={() => setEditing(blankEvent())}>New event</Button></>} />
    {store.error && <div className="next-inline-error" role="alert">{store.error}</div>}
    <Card><div className="next-calendar-toolbar"><strong>{calendarWindow.first.toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</strong><div><Button variant={view === 'month' ? 'primary' : 'ghost'} onClick={() => setView('month')}>Month</Button><Button variant={view === 'agenda' ? 'primary' : 'ghost'} onClick={() => setView('agenda')}>Agenda</Button></div><form onSubmit={event => { event.preventDefault(); void store.quick(quick) }}><input aria-label="Quick add" value={quick} onChange={event => setQuick(event.target.value)} placeholder="Lunch Friday at noon" /><Button type="submit" disabled={!quick.trim()}>Parse</Button></form><Button variant="ghost" onClick={() => void store.syncNow()}>Sync</Button><label className="btn btn-ghost">Import ICS<input hidden type="file" accept=".ics,text/calendar" onChange={event => { const file = event.target.files?.[0]; if (file) void file.text().then(content => store.importIcs(content)).then(setImported) }} /></label>{imported !== null && <small>{imported} imported</small>}</div>
