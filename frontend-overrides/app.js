...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  function applyUIVis(state) {
    ...
    // Text-only emojis toggle. Default is OFF since the /api/emoji proxy
    // serves real glyphs (text-emojis is in UI_VIS_DEFAULT_OFF, so the
    // checkbox defaults unchecked to match) — only an explicit true enables
    // the :name: conversion.
