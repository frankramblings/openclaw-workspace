...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Gary Chat';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to Gary kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'Gary Chat';
    }
