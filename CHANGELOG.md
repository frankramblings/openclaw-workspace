...
- Real steering: restored on the new gateway and verified end to end (`deploy/gateway-patches/claude-cli-steer2.py`). The v1 patch is a no-op on 2026.9.3+ — its anchor is gone because claude-cli became an agent runtime instead of a reply backend, which left the gateway acking steers it then silently dispatched as new runs. Two halves were needed: the workspace must send `queueMode: "steer"` on chat.send (the gateway reads the steer intent from the request, not from `messages.queue.mode`), and the patch must answer "injectable" for a live CLI turn in the reply-run registry. `/api/capabilities` accepts either bundle shape and requires the whole v2 patch before reporting steering available.
- Reply doubling: fixed. On 2026.9.3 the trailing `state:"final"` frame carries every assistant block of the turn joined together, not just the current one, so it fails the `startswith(msg_text)` check; when a tool call landed after the last text block, the relay committed the on-screen text and then appended the whole turn again, rendering and persisting the reply twice. The relay now treats a frame that *ends* with text already on screen as a re-delivery and resets instead of committing (`backend/bridge.py`), covered by a regression test in `backend/tests/test_bridge_relay.py`.
- Reply doubling, the real cause: fixed in the HISTORY mapper, not the live relay. The earlier relay fix above treated the right symptom in the wrong layer — the live SSE stream was already correct, which is why doubling survived it and kept coming back on reload. On 2026.9.3 the gateway closes a claude-cli turn with an EXTRA assistant message carrying every text block of that turn joined together. The gateway's own transcript stores the reply exactly once; `_map_history` was appending that snapshot to the last round, so `/api/history` returned the whole turn a second time and every reload rendered the reply twice, back to back, inside one response. It is now recognised as a re-delivery and dropped (`backend/bridge.py` `_is_turn_snapshot`), on both the tool-using and pure-text paths, matched on whitespace-squashed equality so the gateway's join separator isn't load-bearing. Regression tests in `backend/tests/test_map_history_turn_snapshot.py` are built from a captured real doubled turn.
- Frame tracing: `backend/bridge.py` can record the shape of every gateway chat frame plus the relay's branch decision, off unless `OPENCLAW_WS_FRAME_TRACE` is set or `.data/frame-trace.enabled` exists (checked per call, so it can be armed without a restart). Added because two rounds of reasoning about frame shapes produced a fix that passed its own test and still doubled on screen.
- Steering live-fire (`scripts/livefire-steer.sh`) now measures what it claims: it reconstructs the reply from its SSE deltas (a split `"NUMBER="`/`"42"` used to read as a failure), hands the model an explicitly permitted long-running command, and counts far enough that the turn is still open when the steer lands.
...
### Pillar C3: URL clip (2026-09)

- Clip any public URL into a readable markdown Library document: paste a link into the Library's "Clip URL" prompt, tap the composer's Clip chip when the draft is a bare URL, or hand a URL to the `?action=clip` iOS Shortcut. Re-clipping the same URL updates the existing document in place, with a version snapshot of the previous body, instead of creating a duplicate.
