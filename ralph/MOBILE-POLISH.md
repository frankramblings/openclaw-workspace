...
- [~] Dead decorative controls with `cursor:pointer` but no `data-act`. DONE: email reader `✦ Summarize` → `summarizeEmail` + inline `.m-email-summary`; `✦ AI reply` + reply-bar `✦ Draft` → `composeAiDraft`; reply box + send → `composeReply`. Built a mobile **compose sheet** (`renderComposeSheet` in mobile-sheets.js, mounted in mobile-app.js when `s.composeOpen`) bound to composeTo/Subject/Body + `sendEmail`/`closeCompose`, with a busy "Sending…"/disabled state — verified via harness (reply prefilled, Send wired). STILL DEAD (need decisions, not contained): `.m-gary-card` in More hub (chevron implies nav — candidate target `openCompanion`, but maybe should be a Gary/account detail — product call), calendar `Day` seg (no Day view exists; only Agenda — either build a Day view or drop the "Day" pill).
