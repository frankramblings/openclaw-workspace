"""Behavioral tests for the Live Jobs API (jobs.py): reads the bin/job registry
(a directory of atomic JSON files) and streams it to the SPA. jobs.py never
writes, so there's no separate "sweeper" process — `_read_all()` IS the
sweeper: it derives "stalled" for quiet running jobs and drops terminal jobs
past the retain window on every read. These tests pin that behavior directly
plus the two routes that expose it (`/api/jobs`, `/api/jobs/stream`) and the
per-job log tail route.
"""
import json
import time
...
def test_read_all_marks_stalled_running_job_only_when_quiet_too_long(jobs_dir):
    ...
    _write(jobs_dir, "quiet", status="running", ago=jobs.STALL_SECS + 5)

    by_id = {r["id"]: r for r in jobs._read_all()}
    assert "stalled" not in by_id["fresh"]
    assert by_id["quiet"]["stalled"] >= jobs.STALL_SECS
...
async def test_jobs_stream_first_frame_is_snapshot(jobs_dir):
    ...
    try:
        ...
        await gen.aclose()   # bounded: never touch the 0.4s poll loop after frame 1
