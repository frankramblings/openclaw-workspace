// Pure helpers for the activity trail. Ported 1:1 from
// frontend-overrides/js/redesign/chat-activity-group.js (2026-07-15).
// (1) groupSteps collapses consecutive same-kind COMPLETED steps into groups
// for the expanded view; (2) summarize aggregates a turn for the collapsed
// summary line. Thinking steps and the currently-running step never group.

export type StepKind = 'read' | 'edit' | 'grep' | 'run' | 'web' | 'think' | string
...
export type ActivityItem =
  | { type: 'single'; step: ActivityStep }
  | { type: 'group'; kind: StepKind; id: string; steps: ActivityStep[] }
...
export function groupSteps(steps: ActivityStep[] | null | undefined): ActivityItem[] {
  const items: ActivityItem[] = []
  let run: { kind: StepKind; steps: ActivityStep[] } | null = null
  const flush = () => {
    if (!run) return
...
export function groupLabel(kind: StepKind, count: number): string {
  return (GROUP_LABEL[kind] || GROUP_LABEL.generic)(count)
}

/** { parts:[string], failed:number } for the collapsed summary line. */
export function summarize(steps: ActivityStep[] | null | undefined): { parts: string[]; failed: number } {
