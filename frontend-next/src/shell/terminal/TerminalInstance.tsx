...
export function TerminalInstance({ sessionKey, name, selected, pinned, onSelect, onUnpin }: { sessionKey: string; name: string; selected: boolean; pinned: boolean; onSelect(): void; onUnpin(): void }) {
  ...
  useEffect(() => {
    ...
    const connect = async () => {
      ...
      ws.onclose = (event) => { if (cancelled) return; if (event.code === 1008) { setStatus('disconnected'); return } setStatus('reconnecting'); retry.current = setTimeout(connect, 1_500) }; ws.onerror = () => setStatus('disconnected')
