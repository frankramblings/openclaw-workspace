...
Failure is silence, deliberately. If systemd is unreachable — no user bus, no
`systemctl` on PATH, a timeout — every function here returns empty rather than
raising, because the spec's degradation rule for this observer is "systemd
unavailable → follower off; units fall back to the descendant scan". A
follower that cannot see a unit must report nothing, never guess.
"""
...
def _run(argv: list[str]) -> str:
    """Default runner: stdout, or "" on any failure whatsoever."""
    try:
...
def _guarded(run, argv: list[str]) -> str:
    """An injected runner is test code and may raise; the contract with our
    caller is still silence-on-failure."""
    try:
        return (run or _run)(argv) or ""
...
def list_active(run=None) -> list[str]:
    """Names of active .service units. 4 ms on this box."""
