...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = '__AGENT_NAME__ Chat';
...
export async function loadSessions() {
  try {
    ...
    // Cold launch vs reload: sessionStorage survives reloads/navigation but
    // not a PWA/app restart. On a cold launch we deliberately DON'T restore
    // the last transcript — launch lands on a fresh default-model chat
    // (user choice 2026-06-07); reloads and #hash deep links still resume.
    const _coldLaunch = !sessionStorage.getItem('ody-session-active');
    if (_coldLaunch) sessionStorage.setItem('ody-session-active', '1');
    ...
    if (hasPendingChat) {
      ...
      targetId = currentSessionId;
    } else if (savedId && !_coldLaunch && activeSessions.some(s => s.id === savedId)) {
      targetId = savedId;
    } else if (!_skipAutoSelect && !_coldLaunch && _realSessions.length > 0) {
    ...
    } else if (!_skipAutoSelect && !_coldLaunch && activeSessions.length > 0) {
      ...
      if (!targetId) {
        try {
          ...
          if (dc.endpoint_url && dc.model) {
            // Check if there's already an empty session with this model we can
            // reuse. This backend's records carry no message_count; a session
            // whose `updated` never moved past `created` has never run a turn
            // (the first turn always bumps it via auto-title) — without this,
            // every cold launch would mint another empty chat.
            const emptyDefault = activeSessions.find(s =>
              s.model === dc.model && (s.message_count === 0 ||
                (s.message_count == null && (s.updated - s.created) < 1500))
            );
            if (emptyDefault) {
              targetId = emptyDefault.id;
              // Same mobile treatment as the create path: land in the chat.
              if (window.innerWidth < 768) {
                const sb = document.getElementById('sidebar');
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to __AGENT_NAME__ kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : '__AGENT_NAME__ Chat';
    }
