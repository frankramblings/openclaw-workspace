...
function mConvSemanticHits(s, titleGroups, rowTabindex) {
...
// The model sheet's empty state renders live/chat.js loadModelOptions()'s
// truth: that loader guards concurrent GETs and records a failed GET
// /api/models in state.live.modelsFailed (cleared when a new attempt starts,
// and on success — however late — with its own runtime.render()). So: no
// catalog + no failure flag = a load is in flight ("Loading models…"); no
// catalog + modelsFailed = tap-to-retry. No local timers or attempt windows
// — this module used to keep a 6s heuristic here because the loader was
// silent about failure; it no longer is.
export function renderModelSheet(s) {
  ...
  if (!groups.length) {
    ...
    // it as the retry data-act genuinely retries, no new action needed. A tap
    // while the load is still in flight no-ops inside the loader.
  ...
  const row = (m) => {
    ...
    return `<div class="m-model-row${active ? ' sel' : ''}" data-act="mSetModel" data-arg="${esc(m.id)}" tabindex="0" role="button">
